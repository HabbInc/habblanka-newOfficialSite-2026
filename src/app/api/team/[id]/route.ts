...
export async function PUT(req: Request, context: any) {
  ...
  await db.collection('teamMembers').updateOne({ _id: toObjectId(id) }, { $set: { ...updates, updatedAt: new Date() } })
  const doc = await db.collection('teamMembers').findOne({ _id: toObjectId(id) })
...
export async function DELETE(_: Request, context: any) {
  ...
  const id = params ? (typeof params.then === 'function' ? (await params).id : params.id) : undefined
  const db = await getDb()
  await db.collection('teamMembers').deleteOne({ _id: toObjectId(id) })
