...
export async function PUT(req: Request, context: any) {
  ...
  await db.collection('faqs').updateOne({ _id: toObjectId(id) }, { $set: { ...updates, updatedAt: new Date() } })
  const doc = await db.collection('faqs').findOne({ _id: toObjectId(id) })
...
export async function DELETE(_: Request, context: any) {
  ...
  const id = params ? (typeof params.then === 'function' ? (await params).id : params.id) : undefined
  const db = await getDb()
  await db.collection('faqs').deleteOne({ _id: toObjectId(id) })
